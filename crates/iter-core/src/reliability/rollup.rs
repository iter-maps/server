...
//! unit-testable; the store adapter (in the worker) wires it to disk and the
//! read adapter (`super::store_read`) reads the cold tier back.
//!
...
impl DayType {
    ...
    /// Inverse of [`DayType::token`].
    pub fn from_token(token: &str) -> Option<Self> {
        Some(match token {
            "weekday" => DayType::Weekday,
            "saturday" => DayType::Saturday,
            "sunday-holiday" => DayType::SundayOrHoliday,
            _ => return None,
        })
    }
}
...
/// time. The merge over all history; what the gateway read-endpoint and a future
/// reranker read. Structurally identical to `Tier1`, so the fold reuses the same
/// algebra.
pub type Tier2 = Tier1;

/// Read-side summary derived from an aggregate: percentiles + on-time rate.
/// This is the surface the gateway read-endpoint and a future reranker consume.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct Readout {
    pub count: u64,
...
mod tests {
    ...
    #[test]
    fn tod_bucket_token_round_trip_is_total() {
        for b in [
            TodBucket::Early,
            TodBucket::AmPeak,
            TodBucket::Midday,
            ...
            assert_eq!(TodBucket::from_token(b.token()), Some(b));
        }
        assert_eq!(TodBucket::from_token("not-a-bucket"), None);
    }

    #[test]
    fn day_type_token_round_trip_is_total() {
        for d in [
            DayType::Weekday,
            DayType::Saturday,
            DayType::SundayOrHoliday,
        ] {
            assert_eq!(DayType::from_token(d.token()), Some(d));
        }
        assert_eq!(DayType::from_token("not-a-day-type"), None);
    }
